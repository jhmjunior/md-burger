...
function ModalOrder({ pedido, isDeliveryIncluded }) {
  ...
  for (let i = 0; i < pedido.length; i++) {
    ...
    {
      item.observacoes
        ? (pedidoFinal += `*Possui alguma observação?* ${item.observacoes}%0A%0A`)
  ...
  %0A%0A*Pedido:*%0A${pedidoFinal}*Entrega:* ${hasComboItem ? "Grátis" : "R$ 5,00"} %0A%0A*Total: R$${(somador + (hasComboItem ? 0 : 5)).toFixed(2)}*
  %0A*Forma de pagamento:* ${
    dinheiro
      ? `Dinheiro.%0A*Precisa de troco?* ${
          dinheiro && isTroco
            ? `SIm%0A${
                dinheiro && trocoValue ? `*Para quanto?* ` : null
              }${trocoValue}`
            : `Não`
        }`
      : `${
          cartao
            ...
            : `${pix ? `Pix` : `Opção de pagamento não informada`}`
        }`
  }
  `;
  ...
  return (
    <>
      ...
      <SubMenuDiv>Forma de Pagamento</SubMenuDiv>
      <RadioInput
        dinheiro={dinheiro}
